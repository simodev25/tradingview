...
fibLevel            = input(title="Fib Level", type=input.float, step=0.001 ,defval=0.333, group=g_strategy, tooltip="Used to calculate upper/lower third of candle. (For example, setting it to 0.5 will mean hammers must close >= 50% mark of the total candle size)")
// Filter Settings
...
var broker = av_oandaDemo ? "xbtpractice" : "xbt"
...
if validLong
    ...
    strategy.entry(id="Long", long=strategy.long, when=validLong, comment="(ID:"+longId+"-SL:" + tostring(truncate((t_stop),4)) + "-TP:"+tostring(truncate((t_target),4)) + ")")
...
if validShort
    ...
    strategy.entry(id="Short", long=strategy.short, when=validShort, comment="(ID:"+shortid+"-SL:" + tostring(truncate((t_stop),4)) + "-TP:"+tostring(truncate((t_target),4)) + ")")
...
strategy.exit(id="Long Exit", from_entry="Long", limit=t_target, stop=t_stop, when=strategy.position_size > 0 ,comment ="ID:"+ tostring(longId))
strategy.exit(id="Short Exit", from_entry="Short", limit=t_target, stop=t_stop, when=strategy.position_size < 0,comment ="ID:"+ tostring(shortid))
