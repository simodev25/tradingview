...
class TradingService:
    ...
    def process_order(self, post_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action_type == 'close':
            ...
            position = self.get_position(order.position_id_xtb)
            ...
            if interval:
                ...
                tradingview_signal = self.check_tradingview_signal(symbol, action_type, position_type=action,interval=interval_param)
    ...
    def close_position(self, position_id: int, symbol: str, cmd: int, volume: float, price: float,
        ...
        try:
            ...
            if success:
                ...
                raise Exception(f"Échec de la fermeture de la position {position_id}.")
            return success
        except Exception:
    ...
    def get_position(self, position_id: int, timeout: int = 30, check_interval: int = 5):
        """
        Retrieve the position associated with the given order_id.
        ...
        try:
            while True:
                ...
                if trades:
                    position = [pos for pos in trades if str(pos['position']) == str(position_id)]
                    if position:
                        return position[0]
                    elif time.time() - start_time > timeout:
